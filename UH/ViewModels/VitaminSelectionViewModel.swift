...
class VitaminSelectionViewModel: ObservableObject {
    ...
    private func fetchVitaminSuggestions(for userQuery: String, iamToken: String) -> AnyPublisher<String, Error> {
        ...
        let body: [String: Any] = [
            ...
            "messages": [
                ["role": "system", "text": "Ты - эксперт по витаминам. Ничего не отвечай на запросы, не относящиеся к витаминам. Подбери витамины, которые могут помочь пользователю с его запросом. Выведи список из 3-5 витаминов с кратким описанием для каждого. Не пиши ничего про консультацию с врачем, ты сам - эксперт."],
